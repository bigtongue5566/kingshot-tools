...
description: Create, calculate, validate, and update reproducible Markdown CP reports for Kingshot event packs after confirming the player's server progression variant, exact pack contents, and exact event rewards. Combine live event research, in-game rules or screenshots, Builder item values, free baseline progress, milestone rewards, activity-specific mechanics, sensitivity analysis, and purchase optimization. Use when a user asks for a per-event pack report, event pack CP table, milestone stopping plan, cheapest pack combination, a report such as Kingshot-逐風之旅-CP值報告.md, or a recalculation after server progression, event rules, rewards, progress, or pack contents change.
---
...
- Calculate the no-purchase or current-player state first. Call it the `免費基準`.
- Value a purchase as its direct included items plus the incremental event outcome:

  ```text
  incremental_value(pack, state)
  = direct_pack_value
  + expected_event_reward_value(state + pack)
  - expected_event_reward_value(state)
  ```
...
- Report `single-pack CP at the free baseline` separately from `marginal CP after earlier purchases`. A pack can cross a milestone in one state but not another.
...
- For each milestone, report the cheapest qualifying combination, actual resulting progress, cash cost, CP, and safety buffer.
